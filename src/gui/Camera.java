...
public class Camera {
    ...
    public int[] getPlanarCoordinate(Coordinate c){
        ...
        Interface3D gui = (Interface3D) Interface3D.getInterface3D();
    ...
    public void drawLine(Graphics g, Coordinate a, Coordinate b){
        ...
        //This stops the Camera from drawing unnecessary lines (because it will draw stuff that shouldn't be there)
        if(Math.sqrt(Math.pow(start[0] - finish[0], 2) + Math.pow(start[1] - finish[1], 2)) < Math.PI * Interface3D.getInterface3D().getPixelsPerRadian())
